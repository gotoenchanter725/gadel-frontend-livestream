...
function Sidebar({ active }) {
  ...
  return (
    <div className='hidden sm:flex flex-col p-4 pr-0 sm:pr-2' id='mySidebar'>
      <div className='custom-sidebar-container !m-0 w-[250px] sm:!w-full shadow-lg p-4'>
        <div className='sidebar'>
          ...
          >
            <CopyRightIcon className={`sidebar-icon ${active !== 'settings' && 'inactive'}`} />
            <div className={`sidebar-item ${active === 'settings' && 'sidebar-item-active'}`} >
              Monetization
            </div>
          </div>
          ...
          <div
            className={`sidebar-row ${active === 'logout' && 'sidebar-row-active'}`}
            onClick={handleLogout}
          >
            <PowerSettingsNewRoundedIcon className={`sidebar-icon ${active !== 'logout' && 'inactive'}`} />
            <div className={`sidebar-item ${active === 'logout' && 'sidebar-item-active'}`} >
              Logout
            </div>
          </div>
        </div>
