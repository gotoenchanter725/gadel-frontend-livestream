  ...
  const handleSubmit = async () => {
    if (email === '') {
      ...
      try {
        ...
        setOpen(false)
        if (res && res.payload && res.payload.user.status) {
          const ev = new Event("loggedin");
          ...
          dispatch({
            ...
            payload: res.payload.user.user,
          });
          navigate('/');
        } else {
          setOpenError(true)
        }
        if( res && res.payload && res.payload.user && res.payload.user.code == 100) {
        ...
        <div className="pt-[50px] px-4 w-full flex justify-around items-center">
          <div className="right-container rounded-2xl bg-white w-full max-w-[555px] min-w-[300px] overflow-hidden">
            ...
            <div className="flex flex-col items-center w-full bg-[#D5D5D5] py-4 sm:py-6 px-4 sm:px-8">
              ...
              <div className='text-button-login' onClick={() => navigate('/reset-password')}>
                Fogot Password
              </div>
            </div>
          </div>
